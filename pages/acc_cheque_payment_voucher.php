        ...
        if ((($_POST['dr_amt'] || $POST_cr_amt) > 0) && ($_SESSION['initiate_bank_debit_note']>0)) {
            ...
            if ($_POST['rcved_remining']==$_POST['dr_amt']) {
                add_to_bank_payment($_SESSION['initiate_bank_debit_note'],$date, $proj_id, $_POST['narration'], $_POST['cash_bank_ledger'], 0,
                    $_POST['amount'], $type,$cur_bal,$_POST['paid_to'],$_POST['cheque_id'],$_POST['maturity_date'],$Cheque_of_bank,$manual_payment_no,$_POST['cc_code'],$_POST['subledger_id'],'MANUAL',$ip,$_POST['receipt_date'],$_SESSION['sectionid'],$_SESSION['companyid'],$_SESSION['userid'],$create_date,$now,$day,$thisday,$thismonth,$thisyear,$receive_ledger);
