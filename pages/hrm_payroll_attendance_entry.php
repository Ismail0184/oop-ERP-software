    ...
    if (isset($_POST['submit']) && isset($_FILES['file'])) {
        ...
        if ($_FILES["file"]["size"] > 0) {
            ...
            while (($eData = fgetcsv($file, 10000, ",")) !== FALSE) {
                ...
                $check_query = "SELECT COUNT(*) AS count FROM ZKTeco_basic_data WHERE employee_id = '".$employee_id."' AND clock_time = '".$formatted_date."'";
...
FROM 
    ZKTeco_basic_data a
    JOIN personnel_basic_info p ON a.employee_id = p.PBI_ID where a.status='MANUAL'
GROUP BY 
    ...
    mysqli_query($conn, "UPDATE ZKTeco_basic_data SET status='CHECKED' where status='MANUAL'");
} // if isset submit


if (isset($_POST['clearAllData'])){
    mysqli_query($conn, "DELETE from ZKTeco_basic_data where status='MANUAL'");
}
...
FROM 
    ZKTeco_basic_data a
    JOIN personnel_basic_info p ON a.employee_id = p.PBI_ID where 1 ".$dateConn."
GROUP BY 
    a.employee_id, DATE(a.clock_time)";
} else {
    $sqlQuery = "
