...
if(prevent_multi_submit()){
    ...
    if(isset($_POST['add'])&&($_POST[$unique_master]>0)){
        if($_POST['dist_unit']>$_POST['inStock_pcs']){
            ...
            $sql_gift_on_order=mysqli_query($conn, 'SELECT id from sale_do_details WHERE do_no='.$_SESSION['unique_master_for_SP'].' and item_id='.$_POST['item_id'].' and dealer_code='.$_POST['dealer_code'].' and total_unit='.$_POST['total_unit'].' and total_amt='. $_POST['total_amt'].' and entry_by='.$_POST['entry_by'].' order by id desc limit 1');
...
$unique_master_for_SP = @$_SESSION['unique_master_for_SP'];

if (isset($_REQUEST['id'])) {
    $edit_value=find_all_field(''.$table_detail.'','','id='.$_REQUEST['id'].'');
}
$edit_value_item_id = @$edit_value->item_id;
$edit_value_total_amt = @$edit_value->total_amt;
$edit_value_unit_price = @$edit_value->unit_price;
$edit_value_dist_unit = @$edit_value->dist_unit;
if(isset($_POST['cancel']))
{
    $crud   = new crud($table_master);
    $condition=$unique_master."=".$unique_master_for_SP;
...
$select_dealer_do_SP = @$_SESSION['select_dealer_do_SP'];
$dealer = find_all_field('dealer_info','','dealer_code='.$select_dealer_do_SP);
$_GET_item_id = @$_GET['item_id'];
$item_all= find_all_field('item_info','','item_id="'.$_GET_item_id.'"');
$item_all_t_price = @$item_all->t_price;
$item_all_production_cost = @$item_all->production_cost;
$item_all_d_price = @$item_all->d_price;
$item_all_m_price = @$item_all->m_price;
...
from
sale_do_details a,item_info b where b.item_id=a.item_id and a.do_no='.$unique_master_for_SP.' order by a.id';
...
$COUNT_details_data=find_a_field(''.$table_detail.'','Count(id)',''.$unique_master.'='.$unique_master_for_SP.'');?>
        ...
        <table align="center" style="width: 60%;; font-size: 11px">
            <tr><td>Dealer / Customer / Outlet</td>
                ...
                <td style="vertical-align: middle"><select class="select2_single form-control" style="width:300px; font-size: 11px" tabindex="-1" required="required"  name="dealer_code" id="dealer_code">
                        <option></option>
                        <?php if(isset($select_dealer_do_SP)>0): ?>
                            <?php foreign_relation('dealer_info', 'dealer_code', 'CONCAT(dealer_code," : ", dealer_name_e)', $select_dealer_do_SP, 'dealer_code='.$_SESSION['select_dealer_do_SP']); ?>
                        <?php else: ?>
                            <?php foreign_relation('dealer_info', 'dealer_code', 'CONCAT(dealer_code," : ", dealer_name_e)', $select_dealer_do_SP, 'canceled="YES"'); ?>
                        <?php endif; ?>
                    </select>
                </td>
                <td style="padding:10px;vertical-align: middle">
                    <?php if(isset($select_dealer_do_SP)>0){
                        $status = find_a_field(''.$table_master.'','status','do_no='.$select_dealer_do_SP)
                        ?>
                        ...
                        <?php if($status!=='COMPLETED'){ ?>
                            <a align="center" href="do_challan_view.php?v_no=<?=$select_dealer_do_SP;?>" target="_new"><img src="../../assets/images/print.png" width="25" height="25" /></a>
                        <?php } else { ?>
                            <a target="_blank" href="chalan_view.php?v_no=<?=$select_dealer_do_SP;?>"><img src="../../assets/images/print.png" width="25" height="25" /></a>
                        <?php } ?>
                    <?php } else { ?>
        ...
        <div class="x_panel">
            <div class="x_content">
                <form action="<?=$page?>" name="addem" id="addem" class="form-horizontal form-label-left" method="post">
                    ...
                    <input type="hidden" name="dealer_code" id="dealer_code" value="<?=$select_dealer_do_SP;?>">
                        ...
                        <tr>
                            ...
                            <td style="width: 21%">
                                <select style="width:90%; font-size: 11px" name="do_type" id="do_type"  required class="form-control col-md-7 col-xs-12">
                                    ...
                                    <?php if(isset($unique_master_for_SP)>0): ?>
    ...
    <form action="<?=$page;?>" name="addem" id="addem" class="form-horizontal form-label-left" method="post">
        ...
        <table align="center" style="width:98%; font-size: 11px" class="table table-striped table-bordered">
            ...
            <tr>
                ...
                <td style="width:10%; vertical-align: middle" align="center">
                    <input type="number" id="unit_price" style="width:99%; height:37px; font-size:11px;text-align:center"  step="any" required="required" value="<?=($item_ids>0)? $item_all_d_price : $edit_value_unit_price?>" name="unit_price"  class="form-control col-md-7 col-xs-12" autocomplete="off" class="unit_price" ></td>
                <td style="width:10%; vertical-align: middle" align="center">
                ...
                <td align="center" style="width:10%; vertical-align: middle">
                    <input type="number" id="total_amt" style="width:99%; height:37px; font-size:11px;text-align:center" required="required" step="any"  name="total_amt" value="<?=$edit_value_total_amt?>" class="form-control col-md-7 col-xs-12" readonly autocomplete="off" class="total_amt" ></td>
