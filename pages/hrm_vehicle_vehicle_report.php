...
{   $condition=$unique."=".$$unique;
 ...
 $sql='Select * from vehicle_registration 
         WHERE 
         section_id='.$_SESSION['sectionid'].' and 
             ...
             <div class="x_content">
                 <table class="table table-striped table-bordered" style="width:100%;font-size:12px">
                     ...
                     while($rows=mysqli_fetch_array($result)){
                         ...
                         //echo $days_between3;
                         ?>
                         <tr style="cursor: pointer" onclick="DoNavPOPUP('<?=$rows['id']; ?>', 'TEST!?', 600, 700)">
                             <td style="width:2%; vertical-align: middle"><?=$j=$j+1;?></td>
                             <td style="vertical-align: middle"><a href="print_preview_vehicle.php?vid=<?php echo $rows['id']; ?>" target="_new" style="text-decoration:none"><?php echo $rows['registration_no']; ?></a></td>
                             <td style="vertical-align: middle"><?=$rows['description']; ?><br />Chassis No - <?php echo $rows['chassis']; ?><br />
                                 Engine No - <?=$rows['engine_no']; ?><br />
                             </td>
                             <td style="text-align:center; vertical-align:middle; <?php if( $days_between<1) { echo 'background-color:red';}?>;">
                                 <?php if( $days_between<1) { echo '<span style="color:#FFF; font-weight:bold">Fitness already Expaired</span>'; ?>
                                 <?php } else {?>
                             ...
                             <td style="text-align:center; vertical-align:middle; <?php if( $days_between2<1) { echo 'background-color:red';}?>;">
                                 <?php if( $days_between2<1) { echo '<span style="color:#FFF; font-weight:bold">Tax Token already Expaired</span>'; ?>
                                 <?php } else {?>
                             ...
                             <td style="text-align:center; vertical-align:middle; <?php if( $days_between3<1) { echo 'background-color:red';}?>;">
                                 <?php if( $days_between3<1) { echo '<span style="color:#FFF; font-weight:bold">Insurance already Expaired</span>'; ?>
                                 <?php } else {?>
