...
if(isset($_POST['extract_MAN'])) {

    $_SESSION['MAN_ID']=$_POST['MAN_ID'];
    $man_master = find_all_field('MAN_master','','id='.$_SESSION['MAN_ID']);

    $_POST['vendor_id'] = $man_master->vendor_code;
    ...
    $res = mysqli_query($conn, "select * from MAN_details where m_id=".$_SESSION['MAN_ID']);
...
if(prevent_multi_submit()){
    ...
    if (isset($_POST['confirm'])) {
        ...
        $crud = new crud($table_master);
        $crud->update($unique);
        $update = mysqli_query($conn, "Update MAN_master set po_create_status='Yes' where id='".$_SESSION['MAN_ID']."'");
        $update = mysqli_query($conn, "Update MAN_details set MAN_RCV_STATUS='Done' where m_id='".$_SESSION['MAN_ID']."'");
...
if(isset($_POST['cancel_MAN']))
{   unset($_SESSION['MAN_ID']);} // cancel PI
?>
...
<form action="<?=$page;?>" method="post" name="cloud" id="cloud" class="form-horizontal form-label-left">
    ...
    <table align="center" style="width:50%; font-size: 11px;" class="table table-striped table-bordered">
