...
$title = "External Receipt Vouchers";
$page = "acc_check_external_receipt_voucher.php";
$uniqueICPBL = "receipt_no";
$uniqueLBC = "receipt_no";


if(isset($_GET[$uniqueICPBL])) {
    $jsonUrl = "http://icpbd-erp.com/51816/cmu_mod/page/api/api_receipt_single_voucher.php?receipt_no=".$_GET[$uniqueICPBL]; // Replace with your API URL
    $jsonData = file_get_contents($jsonUrl);
    $data = json_decode($jsonData, true);
} else {
    // icpbl software
    $jsonUrl = "http://icpbd-erp.com/51816/cmu_mod/page/api/api_receipt_vouchers.php"; // Replace with your API URL
    $jsonData = file_get_contents($jsonUrl);
    $data = json_decode($jsonData, true);
}

if(isset($_GET[$uniqueLBC])) {
    $jsonUrl = "http://lbcme.icpbd-erp.com/news/api/api_receipt_single_voucher.php?receipt_no=".$_GET[$uniqueICPBL]; // Replace with your API URL
    $jsonData = file_get_contents($jsonUrl);
...
$receiptVoucherNo = automatic_voucher_number_generate('receipt','receipt_no',1,'1'.$sectionid_substr);
...
if(prevent_multi_submit()) {
    ...
    if (isset($_POST['confirmSaveReceiptICPBL'])) {
        foreach ($data["data"] as $item) {
            ...
            // Construct the URL
            $deleteVoucherUrl = "http://icpbd-erp.com/51816/cmu_mod/page/api/api_receipt_single_voucher_deleted.php?receipt_no=" . $_GET[$uniqueICPBL];
            ...
            <div class="x_content">
                <form  name="addem" id="addem" class="form-horizontal form-label-left" method="post">
                    ...
                    if($GET_status=='UNCHECKED'){  ?>
                        <p>
                            <button style="float: left; margin-left:1%;  font-size: 11px" type="submit" name="deleteICPBL" id="delete" class="btn btn-danger" onclick='return window.confirm("Are you confirm to returned?");'>Delete</button>
...
<div class="col-md- col-sm-12 col-xs-12">
    <div class="x_panel">
        <div class="x_title">
            <h2>External Receipt Voucher (ICPBL)</h2>
        ...
        <div class="x_content">
            <table id="datatable-buttons" class="table table-striped table-bordered" style="width:100%; font-size: 11px">
                ...
                <tr class="bg-primary text-white">
                    ...
                    <th style="vertical-align: middle">Receipt Date</th>
                ...
                <?php if (!empty($data["data"])): ?>
                    <?php foreach ($data["data"] as $item): ?>
                        <tr style="cursor:pointer" onclick="DoNavPOPUPLBC(<?=$item['receipt_no']?>)">
...
<?php if (!isset($_GET[$uniqueLBC])) { ?>
    ...
    <div class="x_panel">
        <div class="x_title">
            <h2>External Receipt Voucher (LBC)</h2>
            <div class="clearfix"></div>
        </div>
        <div class="x_content">
            <table id="datatable-buttons" class="table table-striped table-bordered" style="width:100%; font-size: 11px">
