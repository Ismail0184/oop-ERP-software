...
$title='Inter-Company Receipt Voucher';
...
$page="acc_intercompany_receipt_voucher.php";
...
//for single FG Add...........................
        if (isset($_POST['add'])) {
            ...
            if ((@$_POST['amount_1'] && @$_POST['amount_2']) > 0) {
                ...
                if (($_POST['amount_1']> 0) && (($_POST['ledger_id_1'] && $_POST['ledger_id_2']) > 0) && ($_SESSION['initiate_credit_note']>0)) {
                    add_to_receipt($_SESSION['initiate_credit_note'], '', $proj_id, $_POST['narration'], $_POST['ledger_id_1'], $_POST['amount_1'],
                ...
                if (($_POST['amount_3']> 0) && (($_POST['Ex_debit_ledger'] && $_POST['Ex2_credit_ledger']) > 0) && ($_SESSION['initiate_credit_note']>0)) {
                    add_to_receipt($_SESSION['initiate_credit_note'], 1845854380, $proj_id, $_POST['narration'], $_POST['Ex_debit_ledger'], $_POST['amount_3'],
                        0, 'Debit', 3, $_POST['paid_to'], $_POST['Cheque_No'], $c_date, $_POST['Cheque_of_bank'], $manual_payment_no, $_POST['cc_code'], 0, 'MANUAL', $ip, $_POST['receipt_date'], $_SESSION['sectionid'], $_SESSION['companyid'], $_SESSION['userid'], $create_date, $now, $day
                    ...
                    add_to_receipt($_SESSION['initiate_credit_note'], 1845854380, $proj_id, $_POST['narration'], $_POST['Ex2_credit_ledger'], 0,
                        $_POST['amount_3'], 'Credit', 4, $_POST['paid_to'], $_POST['Cheque_No'], $c_date, $_POST['Cheque_of_bank'], $manual_payment_no, $_POST['cc_code'], 0, 'MANUAL', $ip, $_POST['receipt_date'], $_SESSION['sectionid'], $_SESSION['companyid'], $_SESSION['userid'], $create_date, $now, $day
...
//for Delete..................................
    ...
    $COUNT_details_data=find_a_field(''.$table_receipt.'','Count(id)',''.$recpt_unique.'='.$initiate_credit_note.' and receipt_date not in ("1845854380")');
    ...
    <form action="<?=$page;?>" enctype="multipart/form-data" name="addem" id="addem" style="font-size: 11px" class="form-horizontal form-label-left" method="post">
        ...
        <table align="center" class="table table-striped table-bordered" style="width:98%; font-size: 11px">
            ...
            <tr style="background-color: bisque">
                ...
                <th style="text-align: center">Bank / Debit Ledger <span class="text-danger">(Dr)</span></th>
                <th style="text-align: center">Customer / Credit Ledger <span class="text-danger">(Cr)</span></th>
            ...
            <tr class="success">
                ...
                <td tyle="vertical-align: middle; width: 38%">
                    <select class="select2_single form-control" style="width:99%; font-size: 11px" required="required"   name="ledger_id_2">
                        <option></option>
                        <?=advance_foreign_relation($dealer_info,'');?>
                    </select></td>
                <td rowspan="3" style="width:15%;vertical-align: middle" align="center">
                    <textarea  id="narration" style="width:100%; height:149px; font-size: 11px; text-align:center"  name="narration"  class="form-control col-md-7 col-xs-12" autocomplete="off" ><?=($edit_value_narration!='')? $edit_value_narration : 'Ref: '.$initiate_credit_note.',';?></textarea></td>
                <td align="center" rowspan="2" style="width:10%; vertical-align: middle;">
                    <input type="number" id="amount_1" style="width:99%; font-size: 11px; text-align:center"  value="<?=$edit_value_dr_amt;?>"  name="amount_1" class="form-control col-md-7 col-xs-12" required placeholder="amt 1 & 2" autocomplete="off" step="any" min="1" />
                </td>
            ...
            <tr class="danger">
                ...
                <td tyle="vertical-align: middle" class="bg-danger">
                    <select class="select2_single form-control" style="width:99%" tabindex="-1" required  name="Ex_credit_ledger">
                        ...
                        foreach ($characters as $character) :;?>
                            <option value="<?=$character->ledger_id;?>" <?php if($find_API_intercompany_ledger->client_id=$character->client_id) echo 'selected';?>><?=$character->ledger_name;?></option> <?php endforeach;  ?>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td style="width:10%; vertical-align: middle;" class="bg-danger">
                    <input type="number" id="amount_3" style="width:99%; font-size: 11px; text-align:center"  value="<?=$edit_value_dr_amt;?>"  name="amount_3" class="form-control col-md-7 col-xs-12" autocomplete="off" placeholder="amt 3" step="any" min="1" />
                </td>
            </tr>
            </tbody>
        </table>
