...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO hrm_leave_info (PBI_ID, type, s_date, e_date, total_days, applied_days, reason,status,leave_responsibility_name,recommended_by,approved_by, entry_by, entry_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if ($stmt === false) {
        echo json_encode([
            "status" => "error",
