...
$page="emp_access_unauthorised_osd.php";
$crud      =new crud($table);
...
if(isset($_POST['viewReport'])){
	...
	from '.$table.' r
	WHERE 
	r.authorised_by="'.$_SESSION['PBI_ID'].'" and r.status="'.$required_status.'" order by r.'.$unique.' DESC';
} ?>
...
<?php require_once 'header_content.php'; ?>
    <script type="text/javascript">
        ...
        {
            myWindow = window.open("<?=$page?>?<?=$unique?>="+lk, "myWindow", "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no,directories=0,toolbar=0,scrollbars=1,location=0,statusbar=1,menubar=0,resizable=1,width=950,height=400,left = 250,top = -1");
        }
    </script>
